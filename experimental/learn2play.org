...
***** +Card+ Minion Detection
      ...
      #+begin_src python
class FramesDataset(luigi.task.ExternalTask):
    def output(self):
        return {'train': luigi.LocalTarget('datasets/frames/train'),
                'test': luigi.LocalTarget('datasets/frames/test'),
                'unlabeled': luigi.LocalTarget('datasets/frames/unlabeled')}
...
class DLIBDetector_Minion(luigi.Task):
    ...
    def complete(self):
        if self.retrain:
            ...
            return super(DLIBDetector_Minion, self).complete()
...
***** Secondary Evaluation
      ...
      #+BEGIN_SRC python :tangle utils/eval-dlib.py
import dlib
...
detector = dlib.simple_object_detector('training/minion-detector.svm')

with open('datasets/detector/minions/test.xml') as f:
    root = etree.fromstring(f.read())

imgs = root.xpath('/dataset/images/image')

def box2box(box):
    return {
        'top': int(box.attrib['top']),
        'bottom': int(box.attrib['top']) + int(box.attrib['height']),
        'left': int(box.attrib['left']),
        'right': int(box.attrib['left']) + int(box.attrib['width'])
    }
...
def onto_map(m, box):
    m[box['top']:box['bottom'], box['left']:box['right']] = 1
    return m
...
for img in imgs:
    im = io.imread(img.attrib['file'])
    dets = map(det2box, detector(im))
    real = map(box2box, img.xpath('box'))

    detmap = np.zeros(im.shape[:2])
    realmap = np.zeros(im.shape[:2])

    detmap = reduce(onto_map, dets, detmap)
    realmap = reduce(onto_map, real, realmap)

    labels.append(detmap.reshape(-1))
    reals.append(realmap.reshape(-1))

labels = np.concatenate(labels)
reals = np.concatenate(reals)

precision, recall, thresholds = precision_recall_curve(reals, labels)
...
plt.ylim([0, 1.05])
plt.xlim([0, 1])
plt.title("Pixel-Labeling Accuracy (DLIB Minion Detector). AUC: {0:0.2f}".format(
    average_precision_score(reals, labels)
))
...
**** Extracting Cutouts
     ...
     To effectively test classification, I need to remove detection from the
     critical path. So: the plan is to extract cutouts from all of the images
     in the frames dataset.

     #+name: minion-classifier:task:cutouts
     #+BEGIN_SRC python
class MinionCutouts(luigi.Task):

    def requires(self):
        from card_detector import FramesDataset, DLIBDetector_Minion
        from hearthstone_classifier import HearthstoneModel, TinyFeatures
        return {'frames': FramesDataset(),
                'detector': DLIBDetector_Minion(),
                'model': HearthstoneModel(features=TinyFeatures)}

    def output(self):
        return luigi.LocalTarget('datasets/cutouts/minions')
    ...
    def run(self):
        ...
        def chunk(iterable, n):
            ...
            return it.izip_longest(fillvalue=None, *args)

        def imgpath(p):
            return os.path.join(p, '*.jpg')

        detector = dlib.simple_object_detector(self.input()['detector'].path)
        classifier = joblib.load(self.input()['model'].path)
        paths = it.chain(iglob(imgpath(self.input()['frames']['test'].path)),
                         iglob(imgpath(self.input()['frames']['train'].path)),
                         iglob(imgpath(self.input()['frames']['unlabeled'].path)))

        for ps in chunk(paths, 10):
            ims = np.array(map(io.imread, filter(None, ps)))
            bgrs = [imresize(im[:, :, ::-1], (16, 16)).reshape(-1) for im in ims]
            predictions = np.array(classifier.predict(bgrs))
            print(predictions)

            good = ims[predictions == 1]
            for im in good:
                dets = detector(im)

                for det in dets:
                    cutout = im[det.top():det.bottom(), det.left():det.right(), :]
                    name = os.path.join(self.output().path,
                                        uuid4().get_hex() + '.jpg')
                    io.imsave(name, cutout)

     #+END_SRC

     Total cutouts produced:

     #+BEGIN_SRC sh :exports both
ls -l datasets/cutouts/minions/*.jpg | wc -l
     #+END_SRC
...
**** Clustering Cutouts
     ...
     The next step is to cluster them. Initial tests show that SIFT+FLANN did a
     good job matching minions. Gonna re-use that to build the initial
     clusters.

     #+name: minion-classifier:task:cluster-base
     #+BEGIN_SRC python
class MinionClusters(luigi.Task):

    def additionalDeps(self):
        return []

    def requires(self):
        return [MinionCutouts()] + self.additionalDeps()

    def output(self):
        return luigi.LocalTarget('datasets/clusters/minions_{ident}'.format(ident=self.identifier()))
    ...
    def run(self):
        ...
        cutout_paths = glob(os.path.join(self.input()[0].path, '*.jpg'))
        ...
        for path in cutout_paths:
            matched = False
            im = io.imread(path)
            descriptor = self.describe(im)

            # check for matching cluster
            for idx, cluster in enumerate(clusters):
                for key in cluster:
                    if self.match(key, descriptor):
                        copy(path, os.path.join(out_path, str(idx)))
                        matched = True
                        break
                if matched:
                    break
            ...
            # no cluster matches
            if not matched:
                new_cluster_path = os.path.join(out_path, str(len(clusters)))
                clusters.append([descriptor])
                os.makedirs(new_cluster_path)
                copy(path, new_cluster_path)
     #+END_SRC

     #+name: minion-classifier:task:sift-flann-cluster
     #+BEGIN_SRC python
class SIFT_FLANN_Cluster(MinionClusters):
    threshold = luigi.IntParameter(default=30)

    sift = cv2.SIFT()
    flann = cv2.FlannBasedMatcher({"algorithm": 0, "trees": 5}, {"checks": 50})

    def identifier(self):
        return 'sift+flann_{threshold}'.format(threshold=self.threshold)

    def describe(self, a):
        return self.sift.detectAndCompute(a, None)[1]

    def match(self, a, b):
        matches = [(m, n) for (m, n) in self.flann.knnMatch(a, b, k=2)
                   if m.distance < 0.7 * n.distance]
        return len(matches) > self.threshold
     #+END_SRC

     #+name: minion-classifier:task:sift-hist-vocab
     #+BEGIN_SRC python
class MinionClusters_HistogramVocab(luigi.Task):

    def requires(self):
        return MinionCutouts()

    def output(self):
        return luigi.LocalTarget('training/vocab_minions_sift.npz')
    ...
    def run(self):
        ...
        cutout_paths = glob(os.path.join(self.input().path, '*.jpg'))
        shuffle(cutout_paths)
        cutout_paths = cutout_paths[:1000]

        sift = cv2.SIFT()

        ims = [io.imread(path) for path in cutout_paths]
        descriptors = [sift.detectAndCompute(im, None)[1] for im in ims]
        feats = np.vstack(descriptors)
     ...
     #+name: minion-classifier:task:sift-hist-cluster
     #+BEGIN_SRC python
class MinionClusters_Histogram(MinionClusters):
    sift = cv2.SIFT()
    threshold = luigi.FloatParameter()

    def __init__(self, *args, **kwargs):
        super(MinionClusters_Histogram, self).__init__(*args, **kwargs)
        with self.input()[1].open('r') as f:
            self.centers = np.load(f)['centers']

    def additionalDeps(self):
        return [MinionClusters_HistogramVocab()]

    def identifier(self):
        return 'sift_histogram'

    def describe(self, a):
        return self.histogram(self.sift.detectAndCompute(a, None)[1])

    def match(self, a, b):
        return np.linalg.norm(a - b) < self.threshold
     #+END_SRC

**** Visualizing the Clusters

     #+BEGIN_SRC python :tangle utils/cluster_viz.py
from glob import glob
import os
from matplotlib import pyplot as plt
...
cluster_roots = sys.argv[1:]

for root in cluster_roots:
    print(root)
    plt.figure()
    gs = gridspec.GridSpec(3,3)
    for i, path in enumerate(glob(os.path.join(root, '*.jpg'))[:9]):
        ax = plt.subplot(gs[i])
    ...
    plt.close()
     ...
     Clusters look pretty solid. Looks like sometimes things get split into two
     clusters (onyxia whelps?) gonna try matching against all within a cluster
     to see if that helps the situation.

     Sample clusters:

     #+caption: Note the variety of highlights and effects.
     [[file:./figs/clusters/3.png]]
     #+caption: Note the invariance to the 'Frozen' effect.
     [[file:./figs/clusters/50.png]]

**** Affinity Propagation

     Affinity Propagation is a clustering algorithm that uses a similarity
     matrix to try and learn clusters. It has a reasonably simple-to-use
     implementation in scikit-learn.

     First, I am going to need similarity matrices for various kinds of
     metrics.

     #+name: minion-classifier:task:similarity-matrix
     #+BEGIN_SRC python
class SimilarityMatrix(luigi.Task):
    metric = TaskParameter()
    limit = luigi.IntParameter(default=None)

    def __init__(self, *args, **kwargs):
        super(SimilarityMatrix, self).__init__(*args, **kwargs)

        self.Metric = self.metric()

    def requires(self):
        return MinionCutouts(), self.metric()

    def output(self):
        return luigi.LocalTarget('datasets/similarity/{identifier}.npz'.format(
            identifier=self.Metric.identifier()
        ))
    ...
    def run(self):
        ...
        cutout_paths = glob(os.path.join(self.input()[0].path, '*.jpg'))[:self.limit]
        def images():
            for path in cutout_paths:
                yield io.imread(path)

        descriptions = map(self.Metric.describe, images())
        similarity = np.zeros((len(descriptions), len(descriptions)))
        for ((ia, da), (ib, db)) in it.combinations(enumerate(descriptions), 2):
             similarity[ia, ib] = similarity[ib, ia] = self.Metric.similarity(da, db)
...
class Metric(luigi.Task):
    ...
    @abc.abstractmethod
    def identifier(self):
        "Produces a string identifying the kind of metric"
        pass

    @abc.abstractmethod
    def describe(self, a):
        "Describes a datum. This description is cached for re-use."
        pass

    @abc.abstractmethod
    def similarity(self, da, db):
        "Computes the similarity between two data."
        pass
     #+END_SRC

     #+name: minion-classifier:metric:flann-matches
     #+BEGIN_SRC python
class Sift_Flann_Metric(Metric):
    sift = cv2.SIFT()
    ...
    def similarity(self, da, db):
        matches = [True for (m, n) in self.flann.knnMatch(da, db, k=2)
                   if m.distance < 0.7 * n.distance]
        return len(matches)
     #+END_SRC

     #+name: minion-classifier:metric:sift-hist
     #+BEGIN_SRC python
class Sift_Hist_Metric(Metric):
    sift = cv2.SIFT()

    def __init__(self, *args, **kwargs):
        super(Sift_Hist_Metric, self).__init__(*args, **kwargs)
        # this is bullshit; solve later
    ...
    def histogram(self, feat):
        h = np.zeros((len(self.centers),), dtype='float32')
        for f in feat:
            dist = np.linalg.norm(self.centers - f, axis=1)
            h[dist.argmax()] += 1
        return h / np.linalg.norm(h)
    ...
    def similarity(self, da, db):
     ...
     Now to attempt to learn the clustering.

     #+name: minion-classifier:cluster:affinity
     #+BEGIN_SRC python
class AffinityPropagationCluster(luigi.Task):
    metric = TaskParameter()

    def requires(self):
        return SimilarityMatrix(metric=self.metric)

    def output(self):
        return luigi.LocalTarget('training/clusters/affinity_{ident}.jl'.format(
            ident=self.metric().identifier()
        ))
    ...
    def run(self):
        ...
        joblib.dump(ap, self.output().path)
     #+END_SRC

     And then to apply it.

     #+name: minion-classifier:cluster:apply
     #+BEGIN_SRC python
class ApplyCluster(luigi.Task):
    cluster_type = TaskParameter()
    metric = TaskParameter()

    def requires(self):
        return MinionCutouts(), self.cluster_type(metric=self.metric)

    def output(self):
        return luigi.LocalTarget('datasets/clusters/{cls}_{ident}/'.format(
            cls=self.cluster_type.__name__,
            ident=self.metric().identifier()
        ))
    ...
    def run(self):
        ...
        def label_path(label):
            return os.path.join(self.output().path, str(label))

        def ensure(label):
            if not os.path.exists(label_path(label)):
                os.makedirs(label_path(label))

        cl = joblib.load(self.input()[1].path)
        metric = self.metric()

        cutout_paths = glob(os.path.join(self.input()[0].path, '*.jpg'))[:800]
        ims = [io.imread(p) for p in cutout_paths]
        descs = np.array([metric.describe(im) for im in ims])

        centers = descs[cl.cluster_centers_indices_]

        similarities = np.zeros((len(descs), len(centers)))
        for ((di, d), (ci, c)) in product(enumerate(descs), enumerate(centers)):
            similarities[di, ci] = metric.similarity(d, c)

        labels = np.argmax(similarities, axis=1)

        for (label, path) in zip(labels, cutout_paths):
            ensure(label)
            copy(path, label_path(label))
     #+END_SRC

**** Putting it Together

     #+name: minion-classifier:final
     #+BEGIN_SRC python :tangle learn2play/minion_classifier.py :noweb yes :comments both
import luigi
import numpy as np
import cv2
import os
from hearthstone_classifier import TaskParameter


<<minion-classifier:task:cutouts>>


<<minion-classifier:task:cluster-base>>


<<minion-classifier:task:sift-flann-cluster>>


<<minion-classifier:task:sift-hist-vocab>>


<<minion-classifier:task:sift-hist-cluster>>


<<minion-classifier:task:similarity-matrix>>


<<minion-classifier:metric:flann-matches>>


<<minion-classifier:metric:sift-hist>>


<<minion-classifier:cluster:affinity>>


<<minion-classifier:cluster:apply>>
