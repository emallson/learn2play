...
****** Specifying a Feature Type
       ...
       add a =FeatureParameter()=, which will return a sub-class of =Features=
       or throw an error.

       #+name: param:features
       #+begin_src python
class FeatureParameter(luigi.Parameter):
    def parse(self, x):
        if type(x) == str:
            cls = globals()[x]
        else:
        ...
        if issubclass(cls, Features) and not cls == Features:
            return cls
        else:
            raise ValueError('Unknown features requested.')
...
class HearthstoneModel(luigi.Task):
    ...
    def requires(self):
        ...
        return {
            'vocab': Vocabulary(features=self.features, rate=self.rate),
            'features': [self.features(video=path.rstrip('.partitions'),
                                       rate=self.rate)
                         for path in paths]
        }

    def output(self):
        return luigi.LocalTarget('training/hearthstone_model_{ident}.xml'.format(
            ident=self.features(video='', rate=self.rate).identifier()
        ))
    ...
    def run(self):
        ...
        with self.input()['vocab'].open('r') as f:
            npz = np.load(f)
            centers = npz['centers']
        ...
        np.savez_compressed('svm_data.npz',
                            trainData=trainData,
                            responses=responses)
        svm.train_auto(
            trainData, responses,
